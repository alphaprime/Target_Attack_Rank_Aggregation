...
for t = 1:T
    for s = 1:length(mu)
        ...
        for i = 1:n - 1
            ...
            % Imperfect Feedback
            rank_theta = sort(rand(n(1), 1), 'ascend');

            % Perfect Feedback 
            for j = 1:length(tmp)
                theta_target{t, s, i}(tmp(j)) = rank_theta(spectral_ranking(j));
            end 

            ranking_target{t, s, i} = tmp;

            % Attack Operation
            w_e = Complete_Reversible_Spectral_Target_Attack(theta_target{t, s, i}, w{t, s});
